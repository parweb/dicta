  ...
  const executeAgent = useCallback(
    async (prompt: string, transcriptContext?: string) => {
      ...
      try {
        ...
        // Run agentic loop
        const updatedMessages = await runAgenticLoop(adapter, messages, sysPrompt, tConfig, setState)
  ...
  const continueConversation = useCallback(
    async (followUpPrompt: string) => {
      ...
      try {
        ...
        const messages = [...conversationMessages, buildInitialMessage(followUpPrompt)]
