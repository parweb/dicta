...
export function useAudioRecorder(): UseAudioRecorderReturn {
  ...
  const onAudioReadyCallback = useRef<((audioBlob: Blob) => Promise<void>) | null>(null);
  ...
  const startRecording = useCallback(async (onAudioReady: (audioBlob: Blob) => Promise<void>) => {
    try {
      setIsRecording(true);
      audioChunks.current = [];
